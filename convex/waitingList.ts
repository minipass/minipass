 ...
 */
function groupByEvent(offers: Array<{ eventId: Id<'events'>; _id: Id<'waitingList'> }>) {
        ...
        const { availableSpots } = await ctx.db
            ...
            .then(async event => {
                ...
                const purchasedCount = await ctx.db
                    ...
                    .then(
                        tickets =>
                            tickets.filter(t => t.status === TICKET_STATUS.VALID || t.status === TICKET_STATUS.USED)
                                .length,
                    )
...
export const cleanupExpiredOffers = internalMutation({
    ...
    handler: async ctx => {
        ...
        const expiredOffers = await ctx.db
            .query('waitingList')
            .filter(q =>
                q.and(q.eq(q.field('status'), WAITING_LIST_STATUS.OFFERED), q.lt(q.field('offerExpiresAt'), now)),
            )
            .collect()
        ...
        for (const [eventId, offers] of Object.entries(grouped)) {
            ...
            await ctx.runMutation(internal.waitingList.processQueue, { eventId: eventId as Id<'events'> })
        }
    },
})
