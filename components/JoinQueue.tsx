...
export default function JoinQueue({ eventId, userId }: { eventId: Id<'events'>; userId: string }) {
    ...
    const handleJoinQueue = async () => {
        try {
            ...
            } else if (error instanceof ConvexError && error.message.includes('tickets available')) {
                toast({
                    variant: 'destructive',
            ...
            {(!queuePosition ||
                ...
                (isEventOwner ? (
                    ...
                    <div className="text-center p-4">
                        <p className="text-lg font-semibold text-destructive">Desculpe, este evento está esgotado</p>
                    </div>
                ) : (
                    <div className="space-y-4">
                        {/* Quantity Selector */}
                        <div className="bg-muted text-muted-foreground p-4 rounded-sm">
